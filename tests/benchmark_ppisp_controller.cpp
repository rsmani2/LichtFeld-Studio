 ...
 *   Conv1x1(3→16) + ReLU → MaxPool(3,3) → Conv1x1(16→32) + ReLU →
 *   Conv1x1(32→64) + ReLU → AdaptiveAvgPool(5,5) → Flatten →
 *   Concat(exposure) → Linear(1601→128) + ReLU → Linear(128→128) + ReLU → Linear(128→9)
 */
...
// LibTorch implementation of the same controller architecture
struct LibTorchController : torch::nn::Module {
    LibTorchController() {
        ...
        // FC layers: 64*5*5 + 1 = 1601 → 128 → 128 → 9
